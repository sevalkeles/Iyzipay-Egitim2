...
var _default = router => {
  ...
  // HALİHAZIRDAKİ BİR KARTLA ÖDEME OLUŞTUR VE KARTI KAYDET - THREEDS - CardIndex
  router.post("/threeds/payments/:cartId/:cardIndex/with-registered-card-index", _Session.default, async (req, res) => {
    let {
      cardIndex
    } = req.params;
    if (!cardIndex) {
      throw new _ApiError.default("Card index is required", 400, "cartIndexRequired");
    }
    if (!req.user?.cardUserKey) {
  ...
  router.post("/threeds/payments/:cartId/with-registered-card-token", _Session.default, async (req, res) => {
    ...
    if (!cardToken) {
      throw new _ApiError.default("Card token is required", 400, "cartTokenRequired");
    }
    if (!req.user?.cardUserKey) {
      throw new _ApiError.default("No registered card available", 400, "cardUserKeyRequired");
    }
